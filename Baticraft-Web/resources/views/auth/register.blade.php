        ...
        <div class="row justify-content-center align-items-center">
            ...
            <div class="col-12 col-md-6 offset-lg-1 col-lg-4 order-lg-2 order-1">
                <div class="mb-lg-9 mb-5">
                    <h1 class="mb-1 h2 fw-bold">Mulai Berbelanja</h1>
                    <p>Di Griya Sri Siji Nganjuk</p>
                    <div>Sudah Memiliki Akun? <a href="{{ route('login') }}"> Masuk Disini</a></div>
                </div>
                <!-- form -->
                <form method="POST" action="{{ route('register') }}">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            <!-- input -->
                            <input type="text" class="form-control @error('nama') is-invalid @enderror" id="inputNama" name="nama" placeholder="Nama" required autocomplete="name" autofocus>
                            @error('nama')
                        ...
                        <div class="col-12">
                            <!-- input -->
                            <input type="text" class="form-control @error('no_telpon') is-invalid @enderror" id="inputNoTelp" name="no_telpon" placeholder="No. Handphone" required autocomplete="tel">
                            @error('no_telpon')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-12">
                            <!-- input -->
                            <input type="email" class="form-control @error('email') is-invalid @enderror" id="inputEmail" name="email" placeholder="Email" required autocomplete="email">
                            @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-12">
                            <!-- input -->
                            <input type="password" class="form-control @error('password') is-invalid @enderror" id="inputPassword" name="password" placeholder="Kata Sandi" required autocomplete="new-password" minlength="8">
                            @error('password')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-12">
                            <!-- input -->
                            <input type="password" class="form-control" id="inputPasswordConfirm" name="password_confirmation" placeholder="Konfirmasi Kata Sandi" required autocomplete="new-password" minlength="8">
                        </div>
                        <!-- btn -->
